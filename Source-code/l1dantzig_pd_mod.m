...
%
% 02 Jan 2007: Slightly modified by Michael Saunders, SOL, Stanford Univ.
%              For dense A:
%              1. Store ATRANS = A' to save repeated transposes
%                 (not a great saving).
%              2. Tell linsolve that Hp is SYMMETRIC and POSITIVE DEFINITE
%                 (more significant).

function xp = l1dantzig_pd_mod(x0, A, At, b, epsilon, pdtol, pdmaxiter, cgtol, cgmaxiter)
...
while (~done)
  ...
  if (largescale)
    ...
    w1 = -(1/tau)*( ATRANS*(A*(1./fe2-1./fe1)) + 1./fu2 - 1./fu1 );
    w1p = w1 - (sig12./sig11).*w2;
    Hp = ATRANS*(A*diag(siga)*ATRANS)*A + diag(sigx);
  % [dx, hcond] = linsolve(Hp, w1p);
    opts.SYM    = true;   % Save half the work
   % opts.POSDEF = true;
