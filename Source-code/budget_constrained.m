...
function [Rsq, coeffs_value, Terms_names, p_val] = budget_constrained(Init_set, Exp, N_budget, F_total, R2, Rsq, p_val, p_val_thresh, Terms_indicator, Terms_matrix, y_predicted)
...
while (N <= N_budget) && (F <= F_total)
    ...
    elseif (((Rsq < R2) && all(p_val(2:end) > p_val_thresh)) || ((Rsq > 0.98) && any(p_val(2:end) < p_val_thresh)) || ((Rsq < R2) && any(p_val(2:end) < p_val_thresh)) || ((Rsq > R2) && all(p_val(2:end) > p_val_thresh))) && (Terms_indicator == 1) && (n_levels == 3)
